...
bool ServiceClient::startService() const
{
    ScmHandle scm(OpenSCManagerW(nullptr, nullptr, SC_MANAGER_CONNECT));
    if (scm.get() == nullptr) {
        antivirus::common::log_error(L"OpenSCManagerW завершился ошибкой; могут требоваться права администратора");
        return false;
    }

    ScmHandle service(OpenServiceW(scm.get(), kServiceName, SERVICE_START | SERVICE_QUERY_STATUS));
    if (service.get() == nullptr) {
        antivirus::common::log_error(L"OpenServiceW не открыл AntivirusGuiService");
        return false;
    }
    ...
    if (!StartServiceW(service.get(), 0, nullptr)) {
        ...
        if (error != ERROR_SERVICE_ALREADY_RUNNING) {
            antivirus::common::log_error(L"StartServiceW не запустил AntivirusGuiService");
            return false;
        }
    }
