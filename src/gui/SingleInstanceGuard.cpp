...
SingleInstanceGuard::SingleInstanceGuard()
{
    ...
    if (mutex_ == nullptr) {
        antivirus::common::log_error(L"CreateMutexW не создал защиту единственного экземпляра");
    ...
    if (!primaryInstance_) {
        antivirus::common::log_info(L"Другой экземпляр GUI уже запущен в этом пользовательском сеансе");
    }
}
