...
std::wstring windowsErrorMessage(DWORD errorCode)
{
    ...
    if (wide.empty()) {
        wide = L"Ошибка Windows " + std::to_wstring(errorCode);
    }
...
{
    common::log_info(L"Подготовка запуска GUI в сеансе " + std::to_wstring(sessionId));

    HANDLE userTokenRaw = nullptr;
    if (!WTSQueryUserToken(sessionId, &userTokenRaw)) {
        const DWORD error = GetLastError();
        logWinApiFailure(
            L"WTSQueryUserToken не выполнен для сеанса " + std::to_wstring(sessionId),
            error
        );
        common::log_warning(
            L"Для WTSQueryUserToken служба должна работать от имени LocalSystem"
        );
        return false;
    }
    ...
    if (!DuplicateTokenEx(
        ...
        const DWORD error = GetLastError();
        logWinApiFailure(L"DuplicateTokenEx завершился ошибкой", error);
        return false;
    }
    ...
    if (!CreateEnvironmentBlock(environment.put(), primaryToken.get(), FALSE)) {
        const DWORD error = GetLastError();
        logWinApiFailure(L"CreateEnvironmentBlock завершился ошибкой", error);
        return false;
    }
    ...
    common::log_info(L"Путь к GUI: " + guiPath.wstring());
    common::log_info(L"Рабочая папка GUI: " + guiDir.wstring());
    ...
    if (!guiExists || existsError) {
        common::log_error(
            L"Исполняемый файл GUI не найден или недоступен: " + guiPath.wstring()
        );
        if (existsError) {
            common::log_error(
                L"Ошибка std::filesystem::exists: " + widenUtf8(existsError.message())
            );
        }
        return false;
    }
    ...
    common::log_info(L"Командная строка CreateProcessAsUserW: " + commandLine);
    ...
    if (!created) {
        const DWORD error = GetLastError();
        logWinApiFailure(L"CreateProcessAsUserW не запустил дочерний GUI", error);
        return false;
    }

    Handle threadHandle(processInfo.hThread);

    common::hardenProcessHandleForDemo(processInfo.hProcess, L"AntivirusWinUi.exe (дочерний процесс)", true);

    launched.processHandle = processInfo.hProcess;
    launched.processId = processInfo.dwProcessId;

    common::log_info(
        L"Дочерний GUI запущен, идентификатор процесса=" + std::to_wstring(launched.processId)
    );
