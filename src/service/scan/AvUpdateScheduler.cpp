...
AvUpdateResult AvUpdateScheduler::runUpdateNow()
{
    ...
    if (!storage_.writeDatabase(releaseDate, records)) {
        result.message = L"Не удалось записать обновление антивирусной базы";
    ...
    if (result.updated) {
        result.message = L"Антивирусная база обновлена и перезагружена";
    } else if (result.rolledBack) {
        result.message = L"Обновление антивирусной базы не выполнено; выполнен откат из резервной копии";
    } else {
        result.message = L"Обновление антивирусной базы не выполнено; загружена резервная база";
    }
...
std::wstring AvUpdateScheduler::scheduledReleaseDate() const
{
    ...
    wchar_t buffer[128] = {};
    swprintf_s(buffer,
               L"%04u-%02u-%02u плановое обновление %02u:%02u:%02u",
